...
def prepare_model_config(
    ...
    default_config = {k: v for k, v in model_config.items() if k != "model_params"}
    ...
    final_config = {k: v for k, v in default_config.items()}
    if trial is None:
        final_config["model_params"] = (
            default_config["model_params"] | model_config["model_params"]
        )
        return final_config
        ...
        if isinstance(value, dict):
            if value["param_type"] == "float":
                ...
                config["model_params"][name] = trial.suggest_float(
                    name=f"{model_class}-{name}", low=low, high=high
                )
                ...
                config["model_params"][name] = trial.suggest_categorical(
                    name=f"{model_class}-{name}", choices=value["param_range"]
                )
            else:
    ...
    final_config["model_params"] = (
        default_config["model_params"] | config["model_params"]
    )
    return final_config


def get_fixed_config(model_config: Dict[str, Any]) -> Dict[str, Any]:
    fixed_config = {k: v for k, v in model_config.items() if k != "model_params"}
    fixed_config["model_params"] = dict()
    for k, v in model_config["model_params"].items():
        if not isinstance(v, dict):
            fixed_config["model_params"][k] = v
    fixed_config["model_params"]["random_state"] = model_config["model_params"].get(
        "random_state", 42
    )
    return fixed_config
