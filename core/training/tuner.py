...
class Tuner:
    ...
    def get_objective(self, parent_run_id):
        def objective(trial: optuna.trial.Trial) -> Any:
            child_run_name = (
                f"{self.model_config["model_class"]}_param_set_{trial.number}"
            )
            ...
            model_config = prepare_model_config(
                model_config=self.model_config, trial=trial, return_default_config=False
            )
            self.mlflow_client.log_param(
                run_id=child_run.info.run_id, key="model_config", value=model_config
            )
            ...
            self.mlflow_client.log_metric(
                run_id=child_run.info.run_id, key="val_accuracy", value=val_acc
            )

            train_acc = clf.score(train_data, train_target)
            self.mlflow_client.log_metric(
                run_id=child_run.info.run_id, key="train_accuracy", value=train_acc
            )
            return val_acc
    ...
    def _train_default_model(self):
        ...
        if len(runs) == 0:
            ...
            default_config = prepare_model_config(
                model_config=self.model_config, trial=None, return_default_config=True
            )
            self.mlflow_client.log_param(
                run_id=default_run.info.run_id, key="model_config", value=default_config
            )
            ...
            self.mlflow_client.log_metric(
                run_id=default_run.info.run_id, key="val_accuracy", value=val_acc
            )

            train_acc = clf.score(data=train_data, target=train_target)
            self.mlflow_client.log_metric(
                run_id=default_run.info.run_id, key="train_accuracy", value=train_acc
            )
    ...
    def tune(self):
        ...
        self.mlflow_client.log_param(
            run_id=parent_run_id, key="model_config", value=model_config
        )
        best_value = study.best_value
        self.mlflow_client.log_metric(
            run_id=parent_run_id, key="val_accuracy", value=best_value
        )
